...
import { ArrowUpRight, Database, FileText, Sparkles, Users } from "lucide-react";
    ...
    <div className="mx-auto w-full max-w-[1400px] min-w-0 p-4 sm:p-6 lg:p-8">
        ...
        <div className="flex items-baseline justify-between gap-4">
          <div>
            ...
            <p className="mt-1 max-w-2xl text-[12px] leading-relaxed text-muted-foreground/40">
              {user?.name
                ? `${user.name}, esta capa ahora gira alrededor de los modulos Kimi y un backend simple.`
                : "Aura ahora prioriza los modulos Kimi con un backend simple."}
            </p>
          </div>
      ...
      >
        <div className="mb-3 flex items-center justify-between">
          ...
          >
            Open Kimi chat <ArrowUpRight className="h-3 w-3" />
          </button>
        </div>
      ...
      >
        <ShortcutCard
          title="Kimi chat"
          body="Conversacion directa con Kimi, historial persistido y sin streaming complejo."
          onClick={() => navigate("/kimi/chat")}
        />
        <ShortcutCard
          title="Kimi agents"
          body="Favoritos y configuracion minima para cada perfil Kimi."
          onClick={() => navigate("/kimi/agents")}
        />
        <ShortcutCard
          title="Kimi vault"
          body="Archivos subidos al vault Kimi con preview y texto extraido."
          onClick={() => navigate("/kimi/vault")}
        />
      </motion.div>
    </div>
...
function ShortcutCard({
  ...
  return (
    <button
      onClick={onClick}
      className="rounded-xl border border-border/30 bg-card/20 p-4 text-left transition-all hover:border-border/50 hover:bg-card/35"
    >
      <p className="text-[12px] font-medium uppercase tracking-wider text-muted-foreground/40">
        {title}
      </p>
