...
function normalizePreview(text: string | null | undefined) {
  ...
  return text.trim().slice(0, 2200) || "Empty preview.";
}
...
export default function KimiVault() {
  ...
  return (
    <div className="mx-auto w-full max-w-[1400px] min-w-0 p-4 sm:p-6 lg:p-8">
      <KimiHeader title="Kimi Vault" />

      <section className="rounded-3xl border border-border/35 bg-card/20 p-4 sm:p-5">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            <h2 className="text-[16px] font-medium text-foreground">Files</h2>
            <p className="mt-1 text-[12px] text-muted-foreground/40">
              {files.length} files, {readyFiles} ready.
            </p>
          </div>
        ...
        <div className="mt-4 overflow-hidden rounded-2xl border border-border/25">
          <div className="hidden grid-cols-[minmax(0,1fr)_120px_120px_80px] gap-3 border-b border-border/20 bg-background/40 px-4 py-2 text-[10px] font-medium uppercase tracking-[0.2em] text-muted-foreground/35 md:grid">
            <span>File</span>
            <span>Status</span>
            <span>Remote</span>
            <span />
          ...
          <div className="divide-y divide-border/15">
            {filteredFiles.map(file => (
              <div
                key={file.id}
                className="grid gap-3 px-4 py-3 md:grid-cols-[minmax(0,1fr)_120px_120px_80px]"
              >
